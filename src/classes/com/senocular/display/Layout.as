	...
	public class Layout extends LayoutConstraint {
		...
		internal static function updateChanged(changedList:Dictionary):void {
			...
			for (var element:* in changedList) {
				...
				if (layout.lastRect.x != layout._rect.x || layout.lastRect.y != layout._rect.y) {
				    ...
				    layout.dispatchEvent(new LayoutEvent(LayoutEvent.POSITION_CHANGED));


					// FORCE CHILDREN TO DISPATCH POSITION_CHANGE
					// VIEW HEIRARCHY MAY NOT MATCH LAYOUT HEIRARCHY
					for (var childLayout:* in layout.childList) {
						Layout(childLayout).dispatchEvent(new LayoutEvent(LayoutEvent.POSITION_CHANGED));
					}

				}

				if (layout.lastRect.width != layout._rect.width || layout.lastRect.height != layout._rect.height)
				{
				  changed = true;

				  layout.dispatchEvent(new LayoutEvent(LayoutEvent.SIZE_CHANGED));
				}
				...
				if (changed)
				{
				    ...
				    //layout.dispatchEvent(new Event(EVENT_RESIZE));
		...
		public function set useDefaultChangeHandler(value:Boolean):void {
			...
			if (_useDefaultChangeHandler) {
				//TODO update for EVENT_RESIZE / EVENT_POSITION
		...
		public function getChildList() : Dictionary {
			return childList;
		}

		public function getGlobalPoint(layout : Layout = null) : Point
		{
			if (layout == null)
			{
				layout = this
			...
			{
				rect = layout.rect
				//trace (layout.rect.x + ", " + layout.rect.y)
				p.x += rect.x;
				p.y += rect.y;
		...
		override public function toString():String
		{
			if (this.target)
			{
		      return "[Layout " + this.target + "]" + this.rect + " ";
			}
			else
			{
			  return "[Layout] " + this.rect + " ";
			}
		}
	}
}